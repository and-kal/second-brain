...
- For storing key-value pairs you would use association lists (or: dictionaries), which are just list of pairs:
  ...
  - ```haskell
    ...
    ```
  - usually, you would use the [`Data.Map`](#datamap) module here, which exports a `Map` type and corresponding functions useful for key-value pairs
  - maps are unordered
...
```haskell
data NameOfDataType = { firstName :: String
                      , lastName :: String
                      , age :: Int
                      } deriving (Show)
```
...
The previously defined `NameOfDataType` is a ›value constructor‹, because it takes some values as parameters and gives out a new value. In a similar manner we also have ›type constructor‹, which produce new types:

```haskell
data Maybe a = Nothing | Just a
```

`Maybe` is not a type in itself. Only if we provide some value for `a` (i.e. parametrizing it).
